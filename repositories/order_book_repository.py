...
def create_order_book(session: Session, quantity: int, order_id: int, book_id: int):
    order_book = OrderBook(quantity=quantity, order_id=order_id, book_id=book_id)
    session.add(order_book)
    return order_book

def get_order_book_by_book_and_status(session:Session, book_isbn:int, bookstore_id:int, status:Status):
